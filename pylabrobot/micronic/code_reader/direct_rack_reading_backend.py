"""Direct hardware rack-reading backend for the Micronic rack scanner.

This backend does not call Micronic Code Reader or IO Monitor. It owns the
...
class MicronicDirectRackReaderError(RackReaderError):
  """Raised when direct Micronic hardware control fails."""


class MicronicDirectDriver(Driver):
  """No-op driver for direct Micronic rack-reader hardware.

  The direct rack-reading capability owns the scanner, serial reader, and
  decoder work. A driver is still supplied so the frontend follows the normal
  PLR Device lifecycle.
  """
...
class MicronicDirectRackReadingBackend(RackReaderBackend):
  """Rack-reading backend that controls the Micronic scanner without the OEM app."""
  ...
  def __init__(
    ...
    self.image_dir = Path(image_dir) if image_dir else Path(tempfile.gettempdir()) / "alakascan-direct"
  ...
  def _scan_rack_blocking(self) -> RackScanResult:
    self.image_dir.mkdir(parents=True, exist_ok=True)
    image_path = self.image_dir / f"micronic_direct_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.bmp"
...
def cluster_axis(values: list[float], expected_count: int, tolerance: float) -> list[float]:
  if not values:
    raise MicronicDirectRackReaderError("No decoded barcode positions are available for grid calibration.")
  ...
  if len(means) == expected_count:
    return means
  if len(means) >= 2:
    return [means[0] + index * (means[-1] - means[0]) / (expected_count - 1) for index in range(expected_count)]
  raise MicronicDirectRackReaderError(
...
def decode_well_crop(image, center_x, center_y, cv2, np, zxingcpp, Image, ImageOps) -> Optional[DecodeResult]:
    ...
    for candidate_mask in candidate_masks(mask, cv2, np):
      ...
      for margin in [0.9, 1.0, 1.1, 1.2, 1.35]:
        ...
        for output_size in [60, 80, 100, 120, 160]:
          ...
          warped = cv2.warpPerspective(crop_array, matrix, (output_size, output_size), borderValue=255)
